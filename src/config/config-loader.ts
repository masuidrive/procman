...
import { ConfigWatcher, WatcherDisposal } from './config-watcher';
...
export class ConfigLoader extends EventEmitter {
  ...
  constructor(options: ConfigLoaderOptions = {}) {
    ...
    this.validator = new ConfigValidator({ projectRoot: cwd, collectIssues: this.options.enableReporting });
    this.normalizer = new ConfigNormalizer({ baseDirectory: cwd, expandEnvironmentVariables: true });
    this.watcher = new ConfigWatcher({ interval: 1000, persistent: true });
    this.reporter = new ConfigReporter({ includeSuggestions: true, maxIssues: 100 });
    this.watcher.on('configChanged', (filePath) => this.emit('configChanged', filePath));
  }
  ...
  async load(filePath: string): Promise<ProcmanConfig> {
    ...
    try {
      ...
      if (this.options.enableCache) {
        this.loadedConfigs.set(absolutePath, { config, filePath: absolutePath, loadTime: Date.now() });
      }
      return config;
    } catch (error) {
  ...
  async loadWithReport(filePath: string): Promise<ValidationReport> {
    return this.reporter.generateReportFromValidation(filePath, () => this.load(filePath));
  }
  getLastReport(): ValidationReport | null { return this.reporter.getLastReport(); }
  ...
  clearCache(): void { this.loadedConfigs.clear(); }
  getCached(filePath: string): LoadedConfig | undefined { return this.loadedConfigs.get(path.resolve(filePath)); }
  ...
  private async validateFilePath(filePath: string): Promise<string> {
    ...
    if (stats.size > this.options.maxFileSize) {
      throw createError('CONFIG_VALIDATION_ERROR', {
        ...
        details: { filePath: absolutePath, fileSize: stats.size, maxSize: this.options.maxFileSize },
      });
    }
    return absolutePath;
  }

  async createBackup(filePath: string): Promise<string> { return this.watcher.createBackup(filePath); }
  ...
  watchConfig(filePath: string, callback: (event: 'change' | 'rename', filename?: string) => void): { dispose: () => void } {
    return this.watcher.watchConfig(filePath, callback);
  }
  stopWatching(filePath: string): void { this.watcher.stopWatching(filePath); }
  stopAllWatching(): void { this.watcher.stopAllWatching(); }
  dispose(): void {
  ...
  getResourceUsage() {
    return {
      ...
      memoryUsage: JSON.stringify({ loadedConfigs: Array.from(this.loadedConfigs.values()) }).length,
    };
  }
}
