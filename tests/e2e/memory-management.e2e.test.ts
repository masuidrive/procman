...
describe('Memory Management E2E Tests', () => {
  ...
  describe('Process Memory Limit Auto-Restart', () => {
    test.skipIf(process.env.CI === 'true')('should auto-restart process when max_memory_restart is exceeded', async () => {
      ...
      console.log(`Process will allocate 150MB total, limit is 30MB, default check interval is 30s`);
      
      // Wait for memory allocation to complete first (3 × 300ms + buffer)
      await sleep(2000);
      console.log('Memory allocation phase should be complete, starting restart monitoring...');
      ...
      while (!restartDetected && (Date.now() - startTime) < maxWaitTime) {
        ...
        const currentPidMatch = currentStatus.stdout.match(/memory-test-app.*?(\d+)/);
        const currentPid = currentPidMatch ? currentPidMatch[1] : null;
        
        console.log(`Memory check: PID ${currentPid} (original: ${initialPid}), elapsed: ${Date.now() - startTime}ms`);
        
        // Check if restart occurred (PID changed)
        if (currentPid && currentPid !== initialPid) {
          console.log('Restart detected! PID changed from', initialPid, 'to', currentPid);
      ...
      if (!restartDetected) {
        console.log('No restart detected yet, waiting additional 10 seconds for memory check cycle...');
        await sleep(10000);
      }
      ...
      const hasRestarted = finalStatus.stdout.match(/Restarts:\s*[1-9]/) || restartDetected;
      expect(hasRestarted).toBeTruthy();
      
      console.log(`Test completed successfully: Restart detected=${restartDetected}, Final status contains restarts=${!!finalStatus.stdout.match(/Restarts:\s*[1-9]/)}`);
